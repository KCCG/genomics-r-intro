...
We are likely to want to analyse this data set from several different angles.
Using a separate script for each type of analysis will help keep up organised.
But all of these different analyses will need to use the same data.
So let's create a little script whose sole purpose will be to load the data into R.
Then we can call this script as the first step of our subsequent analyses.
